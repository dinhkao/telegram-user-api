...
from server_app.inventory_routes import _box_summary, _latest_stock_changes, _time_key
...
class LatestStockChanges(Base):
    def setUp(self):
        super().setUp()
        self.conn.execute(
            "CREATE TABLE audit_events (id INTEGER PRIMARY KEY, ts TEXT, action TEXT, payload_json TEXT)"
        )

    def test_latest_real_stock_change_wins_and_code_is_normalized(self):
        events = [
            ("2026-07-15T10:00:00Z", "box.created", {"product_code": "k10"}),
            ("2026-07-15T11:00:00Z", "box.allocated", {"product_code": "K10"}),
            # Chuyển kho mới hơn nhưng KHÔNG đổi tổng tồn → không được tính.
            ("2026-07-15T12:00:00Z", "box.moved", {"product_code": "K10"}),
        ]
        self.conn.executemany(
            "INSERT INTO audit_events (ts, action, payload_json) VALUES (?, ?, ?)",
            [(at, action, json.dumps(payload)) for at, action, payload in events],
        )
        self.conn.commit()

        self.assertEqual(_latest_stock_changes(self.conn)["K10"], "2026-07-15T11:00:00Z")

    def test_time_comparison_respects_timezone_offsets(self):
        self.assertGreater(
            _time_key("2026-07-15T20:00:00+07:00"),
            _time_key("2026-07-15T12:00:00Z"),
        )
