"""Test hàng rào PHẠM VI API của vai trò `chat_luong` (2 trang báo cáo ảnh hằng ngày:
chất lượng mâm kẹo + vệ sinh khu vực).

Đây là chặn THẬT ở middleware — không phải ẩn menu. Nguyên tắc: MẶC ĐỊNH TỪ CHỐI,
chỉ mở đúng /api/auth*, /api/quality*, /api/areas* và
/api/media/{quality_report,quality_image,area_report,area_image}*.
"""
...
class QualityOnlyScopeTest(unittest.TestCase):
    def test_duong_cua_trang_chat_luong_deu_qua(self):
        for m, p in [
            ...
            # trang vệ sinh khu vực — mở 2026-08-19 (cùng người chụp 2 loại báo cáo)
            ("GET", "/api/areas"),
            ("GET", "/api/areas/3"),
            ("POST", "/api/areas"),
            ("POST", "/api/areas/3/report"),
            ("GET", "/api/media/area_report/3/images"),
            ("POST", "/api/media/area_report/3/images"),
            ("POST", "/api/media/area_image/9/comments"),
            ("GET", "/api/auth/me"),
            ("POST", "/api/auth/login"),
        ]:
    ...
    def test_khong_lot_bang_tien_to_gan_giong(self):
        ...
        for p in [
            ...
            "/api/areas-secret",
            "/api/area/1",                 # số ít: không phải nhánh của trang vệ sinh
            "/api/media/area_report_x/1/images",
            "/api/media/xarea_image/1/comments",
        ]:
...
class ApiScopeDeniedTest(unittest.TestCase):
    ...
    def test_chat_luong_bi_chan_ngoai_pham_vi(self):
        ...
        self.assertTrue(api_scope_denied(QUALITY_ONLY_ROLE, "GET", "/api/media/order/1/images"))
    ...
    def test_chat_luong_qua_duoc_trong_pham_vi(self):
        ...
        self.assertFalse(api_scope_denied(QUALITY_ONLY_ROLE, "POST", "/api/areas/3/report"))
        self.assertFalse(api_scope_denied(QUALITY_ONLY_ROLE, "GET", "/api/media/area_report/1/images"))
    ...
    def test_ws_event_chi_nhan_trang_chat_luong(self):
        ...
        for ev in ("quality_changed", "area_changed", "ping", "app_reload"):
        ...
        for ev in ("order_changed", "orders_changed", "notif_added", "cashbox_changed",
                   "customer_changed", "inventory_changed", "bean_changed",
