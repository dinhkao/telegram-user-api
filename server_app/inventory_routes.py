...
async def place_delete_handler(request: web.Request):
    ...
    def _run():
        ...
        try:
            _ensure(conn)
            # Vị trí đang có phiếu KIỂM KHO nháp → xoá vị trí làm phiếu mồ côi. Cấm.
            # (bảng có thể chưa tồn tại trong DB cũ → try/except bỏ qua guard)
            try:
                has_draft = conn.execute(
                    "SELECT 1 FROM inventory_stocktakes WHERE place_id = ? AND status = 'draft' LIMIT 1",
                    (pid,)).fetchone()
            except Exception:
                has_draft = None
            if has_draft:
    ...
    if status == "draft":
        return web.json_response({"ok": False, "error":
            "Vị trí đang có phiếu kiểm kho nháp — hoàn tất/huỷ phiếu trước"}, status=400)
    from server_app.realtime import emit_inventory_changed, emit_box_changed
    emit_inventory_changed()
    emit_box_changed()
    return web.json_response({"ok": True})
...
async def box_delete_handler(request: web.Request):
    """Xoá HẲN 1 thùng — admin xoá mọi thùng; NGƯỜI DÙNG THƯỜNG chỉ xoá thùng
    thuộc phiếu NHẬP HÀNG đang MỞ khoá (gỡ dòng nhập sai / dọn sau hủy chốt —
    mở cùng nhập kho 2026-07-17). Cấm nếu khoá xoá (_box_delete_lock: đã
    xuất/chuyển, phiếu nhập đang chốt, phiếu trả đã xử lý)."""
    from server_app.order_api_common import is_admin_request
    ...
    if status == "forbidden":
        return web.json_response({"ok": False, "error":
            "Chỉ xoá được thùng của phiếu nhập hàng đang mở — thùng khác cần admin"}, status=403)
...
async def box_detail_handler(request: web.Request):
    ...
    def _run():
        ...
        try:
            ...
            src_return = None
            if box.get("source_return_id"):
                # thùng tạo từ phiếu TRẢ HÀNG → link nguồn về #/tra-hang/:id
                from return_store import get_return as _get_return
                rt = _get_return(conn, int(box["source_return_id"]))
                if rt:
                    src_return = {"id": rt["id"], "created_at": rt.get("created_at")}
