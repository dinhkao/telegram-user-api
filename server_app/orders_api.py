...
def _build_order_row(r) -> dict:
    ...
    try:
        ...
        # Tên người HOÀN THÀNH từng bước (badge 5 bước hiện tên thay nhãn HĐ/Soạn/…)
        task_bys = []
        for tt in ("ban_hd", "soan_hang", "giao_hang", "nop_tien", "nhan_tien"):
            st = ts.get(tt, {}) or {}
            by = st.get("by")
            task_bys.append(USER_NAMES.get(str(by), str(by)) if by and st.get("done") else "")
    except Exception:
        giao_by = nop_by = ""
        task_bys = ["", "", "", "", ""]
    inv = j.get("invoice") or []
    # mã SP hiển thị = mã HIỆN HÀNH (map cache 30s; blob giữ snapshot)
    from order_store.display import display_item, display_maps
    _maps = display_maps()
    ...
    # Badge VAT trên card: đơn có HĐ điện tử VNPT (nháp/đã phát hành)
    _vnpt = j.get("vnpt_invoice") or None
    # + số HĐ (chỉ khi đã phát hành — nháp mang số 0); "00000338" → 338
    vnpt = None
    if _vnpt:
        try:
            _no = int(str(_vnpt.get("invoice_no") or 0).strip() or 0)
        except ValueError:
            _no = 0
...
async def order_detail_handler(request: web.Request):
    ...
    try:
        ...
        try:
            ...
            j = {}
        # mã/tên SP hiển thị = bản HIỆN HÀNH (sp_id → products; blob giữ snapshot)
        from order_store.display import resolve_invoice_display
        for arr in ("invoice", "invoice_items"):
            if isinstance(j.get(arr), list) and j.get(arr):
                j[arr] = resolve_invoice_display(j[arr], conn)
