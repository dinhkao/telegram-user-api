...
def _build_html(data: dict, thread_id: int, elapsed_ms: float) -> str:
    ...
    def ts(entry):
        if not isinstance(entry, dict):
            return "—"
        status = "✅" if entry.get("done") else "🔘" if entry.get("skip") else "❌"
