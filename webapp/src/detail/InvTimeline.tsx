...
function EventRow({ it, idx }: { it: PlaceTLItem; idx: number }) {
  ...
  const act = (() => {
    switch (it.kind) {
      ...
      case "created": return <>nhập mới {boxRef} <b>{soVN(amt)}</b>{u} từ {
      ...
      case "return_in_removed": return <>gỡ <b>{soVN(amt)}</b>{u} hàng khách trả khỏi {boxRef}{it.return_id ? <> (<a class="pt-inl" href={`#/tra-hang/${it.return_id}`}>phiếu trả #{it.return_id}</a>)</> : null}</>;
