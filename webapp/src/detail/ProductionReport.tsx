// Báo cáo sản xuất theo thợ — CHỈ XEM ở trang chi tiết. Sửa → trang riêng
// (#/san_xuat/:id/bao-cao, ProductionReportEdit) để khoá 1 người sửa cùng lúc.
...
export function ProductionReport({ threadId, slip }: { threadId: string; slip: ProdSlip }) {
  ...
  return (
    <section class="card">
      ...
      {!open && rows.length > 0 && (
        <div class="muted small">Tổng: <b>{soVN(rep!.grand_total)}</b> · {rows.length} thợ</div>
      )}

      {open && (
        <>
          <div class="row space" style={{ alignItems: "center", marginBottom: "6px" }}>
            <div class="prod-report-meta" style={{ margin: 0 }}>
