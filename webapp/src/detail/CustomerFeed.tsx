      ...
      for (let i = 0; i < pts.length - 1; i++) {
        ...
        if (pts[i + 1].st === "ok") {
          ...
          if (low) {
            ...
            // LUÔN lặp số 0 (kể cả quãng ngắn); kẹp y để không đè lên mốc 0 gốc.
            // Line kéo xuống ĐÚNG TÂM chấm lặp (nối liền, không hụt).
            const zy = Math.min(low.y + 18, pts[i + 1].y - 14);
            dl.push({ x, y1: pts[i].y, y2: zy, st });
            segMeta.push({ topEl: pts[i].el, botEl: low.el, y1: pts[i].y, y2: zy });
            repz.push({ x, y: zy, t: pts[i + 1].el.textContent || "0" });
          }
          continue;
        }
