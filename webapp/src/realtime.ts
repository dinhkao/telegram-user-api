...
export type RealtimeEvent =
  ...
  | { type: "bean_changed" }
  | { type: "forecast_changed"; id: string }
...
const _SERVER_EVENTS = new Set([
  ...
  "cashbox_changed", "app_reload", "area_changed", "bean_changed", "forecast_changed",
]);
...
export function eventMatchesBase(base: string, e: RealtimeEvent): boolean {
  ...
  // Kho đậu: event KHÔNG mang id phiếu → mọi luồng ảnh/trao đổi của phiếu đang mở tải lại.
  if (e.type === "bean_changed") return base.includes("/bean_slip/") || base.includes("/bean_stocktake/");
