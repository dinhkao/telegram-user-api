...
/** 1 đơn đã ẩn khỏi trang thu tiền (cờ bypass_debt) — chỉ trả khi hỏi ?hidden=1. */
export type HiddenDebtOrder = {
  thread_id: number; created?: string | null; total: number; debt: number; label?: string;
};
export type Debtor = {
  ...
  hidden_count?: number; hidden_amount?: number;   // đơn đã ẩn khỏi trang thu tiền
  hidden_only?: boolean;                           // khách CHỈ còn đơn ẩn (collectable = 0)
  hidden_orders?: HiddenDebtOrder[];
};
 ...
 *  `withHidden` = kèm các đơn đã ẩn khỏi trang thu tiền (và khách chỉ còn đơn ẩn). */
export async function getDebtors(withHidden = false): Promise<{
  debtors: Debtor[]; total_collectable: number; count: number;
  hidden_count: number; hidden_total: number; hidden_customer_count: number;
}> {
  const d = await getJSON(`/api/collect/debtors${withHidden ? "?hidden=1" : ""}`, { cache: false });
  return {
    debtors: d.debtors || [], total_collectable: d.total_collectable || 0, count: d.count || 0,
    hidden_count: d.hidden_count || 0, hidden_total: d.hidden_total || 0,
...
export async function listProduction(page = 1, kind?: "san_xuat" | "dong_goi", day?: string, mismatch?: boolean): Promise<ProdListResp> {
  const d = await getJSON(`/api/production?page=${page}${kind ? `&kind=${kind}` : ""}${day ? `&day=${day}` : ""}${mismatch ? "&mismatch=1" : ""}`);
...
export type PayrollRow = {
  ...
  // TRỪ BHXH — khoản TRỪ hằng tháng, cùng luật kế thừa với mốc (salary_store/bhxh.py):
  ...
  // LƯƠNG CHỜ HÀNG — tiền trả cho thời gian ngồi chờ nguyên liệu/hàng về, văn phòng
  // gõ tay từng tháng (bấm ô để nhập). Khoản CỘNG, đã vào thuc_lanh, KHÔNG kế thừa.
  cho_hang: number;
  // NGÀY CÔNG gõ tay đè máy chấm công: cong = số ĐANG DÙNG, cong_auto = số máy chấm
  // quy ra, cong_manual = đang bị đè. SỐ TRỪ ẨN khỏi lương SP: tru_an + luong_goc
  // (lương SP TRƯỚC khi trừ). 2 số này CHỈ hiện ở bảng lương của văn phòng — phiếu
...
export async function setPayrollAdjust(ym: string, worker_id: number, patch: { thuong?: number; note?: string; weekly?: boolean; thuong_cc?: boolean; thuong_vs?: boolean; cho_hang?: number; tru_an?: number; cong_override?: number | null; monthly_salary?: number; bhxh?: number | null }): Promise<PayrollMonth> {
...
export async function listAllAdvances(ym: string): Promise<SalaryAdvance[]> {
  const d = await getJSON(`/api/payroll/advances?ym=${encodeURIComponent(ym)}`, { cache: false });
  return d.advances || [];
}
...
export async function setPayrollAdvanceNote(ym: string, id: number, note: string): Promise<PayrollMonth> {
  return postJSON(`/api/payroll/advance/${id}/note`, { ym, note });
}
...
export type ReportSlip = {
  ...
  report?: {
    workers: { name: string; cay: number; money: number; allowance: number;
               ...
               days?: { ymd: string; cay: number; money: number;
                        items: { code: string; cay: number; wage: number; money: number;
                                 start?: string; end?: string; thread_id?: number }[] }[] }[];
...
// ── Timeline biến động 1 ĐƠN HÀNG ──
export type OrderTLPart = { t: string; href?: string };
export type OrderTLItem = {
  ts: number; at: string; kind: string; dir: "in" | "out" | "neutral"; label: string;
  parts: OrderTLPart[]; actor?: string; amount?: number; remaining: number;
  changes?: { label: string; old?: string; new?: string }[]; image_id?: number | null; task?: string;
};
export type OrderTimeline = {
  order: { thread_id: number; text: string; total: number; paid: number; remaining: number;
           customer_key?: string | number | null; customer_name?: string | null;
           kv_code: string; created: string; ngay_giao: string };
  items: OrderTLItem[]; truncated: boolean;
};
export async function getOrderTimeline(threadId: string | number): Promise<OrderTimeline | null> {
  const d = await getJSON(`/api/order/${Number(threadId)}/timeline`, { cache: false });
  return d.ok ? d : null;
}
...
export type InvProductLink = { id?: number; code: string; name: string; cost_price: number; unit?: string;
  // thông số SX: prod_mam = số CÂY trên 1 mâm (vào công thức tổng cây báo cáo SX),
  // prod_luong = lượng 1 mẻ. null/undefined = chưa đặt → lùi về SP_INFO config cũ.
  prod_mam?: number | null; prod_luong?: number | null; can_produce_directly?: boolean; can_package?: boolean; self_container?: boolean; min_stock?: number; can_sell?: boolean; can_purchase?: boolean; kv_id: number | null; kv_full_name: string | null; kv_synced_at: string | null; linked: boolean };
...
export type StockDemandIngredient = { code: string; name: string; unit: string; need: number; stock: number; enough: boolean; shortfall: number; ratio?: number; min_stock?: number; children?: StockDemandIngredient[] };
export type StockDemandLine = { code: string; name: string; unit: string; need: number; stock: number; enough: boolean; shortfall: number; orders: number; orders_detail?: StockDemandOrder[]; ingredients?: StockDemandIngredient[]; cay_per_mam?: number; can_direct?: boolean; min_stock?: number };
...
export async function updateProduct(code: string, patch: { unit?: string; name?: string; note?: string; prod_mam?: number | string; prod_luong?: number | string; can_produce_directly?: boolean; can_package?: boolean; self_container?: boolean; min_stock?: number; can_sell?: boolean; can_purchase?: boolean; aux_required?: boolean; bulk_unit_id?: number | null; display_unit_id?: number | null; stocktake_unit_id?: number | null }): Promise<InvProductLink | null> {
