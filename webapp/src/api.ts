...
export type InvProductLink = { code: string; name: string; cost_price: number; unit?: string; kv_id: number | null; kv_full_name: string | null; kv_synced_at: string | null; linked: boolean; is_material?: boolean };
...
export async function setRecipeLine(code: string, ingredientCode: string, ratio: number, optional = false): Promise<RecipeLine> {
  const d = await postJSON(`/api/products/${encodeURIComponent(code)}/recipe`, { ingredient_code: ingredientCode, ratio, optional }, { queueable: false });
  return d.line;
}
...
export async function updateProduct(code: string, patch: { unit?: string; name?: string; note?: string; is_material?: boolean }): Promise<InvProductLink | null> {
...
export async function listTasks(filter: string, page = 1, q = ""): Promise<{ tasks: Task[]; total: number; total_pages: number; counts: TaskCounts; today: string }> {
  const d = await getJSON(`/api/tasks?filter=${encodeURIComponent(filter)}&page=${page}${q ? `&q=${encodeURIComponent(q)}` : ""}`, { cache: false });
