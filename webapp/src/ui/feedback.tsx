...
type Confirm = { msg: string; okLabel: string; cancelLabel: string; danger: boolean; resolve: (v: boolean) => void };
...
export function confirmDialog(msg: string, opts: { okLabel?: string; cancelLabel?: string; danger?: boolean } = {}): Promise<boolean> {
  return new Promise((resolve) => {
    // Nếu đang có hộp khác → huỷ hộp cũ (trả false) để không kẹt.
    if (current) current.resolve(false);
    current = { msg, okLabel: opts.okLabel ?? "Đồng ý", cancelLabel: opts.cancelLabel ?? "Huỷ", danger: !!opts.danger, resolve };
    emitCf();
  });
}
