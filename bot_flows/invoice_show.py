"""bot_flows/invoice_show.py — Render invoice HTML from cached session."""
...
async def handle_show_invoice(bot, event, s):
    ...
    kb = keyboards.build_inline_invoice_keyboard(has_items=bool(invoice), has_kv=bool(s.kv_invoice_id))
    invoice_html = ""
    if invoice:
        lines = []
        for item in invoice:
            if not isinstance(item, dict):
                continue
            code, qty, price = item.get("sp") or "", item.get("sl") or 0, item.get("price") or 0
            total = qty * price
            parts = [code]
            if qty > 0:
...
def _build_summary_html(s) -> str:
    invoice_total = sum((it.get("price") or 0) * (it.get("sl") or 0) for it in s.invoice or [] if isinstance(it, dict))
    discount, pvc, vat, kh_debt = s.discount, s.pvc, s.vat, s.kh_debt
    total_payments = sum(p.get("amount", 0) for p in (s.payments or []) if isinstance(p, dict))
    ...
    if (invoice_total or discount or pvc or vat) and order_total != invoice_total:
        entries.append(("🧾 Tổng đơn:", fmt(order_total), True))
    if kh_debt != 0:
        entries.append(("💳 Nợ trước:", fmt(kh_debt), True))
    entries.append(("💯 Tổng TT:", fmt(final_total), True))
    if total_payments > 0:
        entries.append(("💸 Đã trả:", fmt(total_payments), True))
    elif s.task_status:
        entries.extend(_build_payment_status(s.task_status))
    money_values = [v for _, v, is_money in entries if is_money]
    max_len = max((len(v) for v in money_values), default=0)
    lines = []
    for label, value, is_money in entries:
        lines.append(label)
        if is_money:
            pad = " " * (10 + max(0, max_len - len(value)))
            lines.append(f"<code>{pad}{value}</code>")
        else:
            lines.append(value)
    return "\n".join(lines)

def _build_payment_status(task_status: dict) -> list:
    entries = []
    ...
    if nop.get("done"):
        ...
        icon = "✅" if done else "❌"
        if mode == "gui_toa":
            entries.append(("📄 Toa:", f"{icon} {'Đã' if done else 'Chưa'} gửi toa", False))
        elif mode == "kiem_tra_ck":
            entries.append(("🏦 CK:", f"{icon} {'Đã' if done else 'Chưa'} kiểm tra CK", False))
        else:
            entries.append(("💵 Nhận:", f"{icon} {'Đã' if done else 'Chưa'} nhận tiền", False))
    else:
