"""bot_flows/payment.py — Nhận tiền payment flow."""
...
async def start_payment_flow(bot, event, s):
    ...
    try:
        totals = await post_json(f"{USER_API_BASE}/api/order/totals", {
            "thread_id": s.thread_id, "use_initial_debt": False})
...
async def handle_payment_text(bot, event, s, text):
    ...
    if pf.get("step") == "ask_amount":
        amt = pf.get("suggested") if txt.startswith("dùng số tiền") else None
        if not amt:
        ...
        if amt and amt > 0:
            pf["amount"] = amt
            pf["step"] = "ask_method"
            await event.reply(f"Số tiền: {nf(amt)}. Chọn phương thức:",
                buttons=keyboards.build_payment_methods_keyboard())
            return
        await event.reply("Số tiền không hợp lệ.", buttons=[[Button.text("Huỷ")]])
        return
    if pf.get("step") == "ask_method":
        is_tm = txt == "tiền mặt"
        is_ck = txt in ("chuyển khoản", "chuyen khoan")
        if not is_tm and not is_ck:
            await event.reply("Hãy chọn: Tiền mặt hoặc Chuyển khoản.",
                buttons=keyboards.build_payment_methods_keyboard())
            return
        if not s.thread_id:
            s.pay_flow = None
        ...
        try:
            ...
            resp = await post_json(f"{USER_API_BASE}/api/order/payment/{method}", {
                "thread_id": s.thread_id, "amount": amt, "user_id": s.user_id})
            debt_note = ""
