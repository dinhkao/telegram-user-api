...
def generate_receipt_html(customer_name: str, thread_id: int, old_debt: int | None, payment_amount: int, new_debt: int | None) -> tuple[str, str]:
...
async def _enqueue_html_to_png(html: str, chat_id: int, thread_id: int, client=None) -> None:
    if client is None:
        log.warning("No Telegram client — skipping receipt PNG render")
        return
    try:
        from firebase_html_to_png import render_and_send_html
        await render_and_send_html(client, html, chat_id, thread_id)
        log.info("Receipt rendered & sent: thread=%d", thread_id)
    except Exception as e:
        log.warning("Receipt PNG render failed: %s", e)


async def send_payment_receipt(client, thread_id: int, customer_name: str, payment_amount: int, old_debt: int | None = None, new_debt: int | None = None) -> None:
    html, file_stamp = generate_receipt_html(customer_name, thread_id, old_debt, payment_amount, new_debt)
    try:
        await _enqueue_html_to_png(html, ORDER_GROUP_ID, thread_id, client)
    except Exception as e:
    ...
    try:
        ...
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(html)
        await client.send_file(ORDER_GROUP_ID, file_path, caption=f"📄 Phiếu thu — {customer_name} — {_format_money(payment_amount)}", reply_to=thread_id, force_document=True)
        try:
            os.remove(file_path)
        except OSError:
            pass
    ...
    log.info("Payment receipt processed: thread=%d customer=%s amount=%s", thread_id, customer_name, payment_amount)
