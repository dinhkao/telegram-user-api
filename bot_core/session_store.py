"""bot_core/session_store.py — SQLite-backed persistence for in-progress sessions."""
...
def _conn() -> sqlite3.Connection:
    ...
    if _connection is None:
        _connection = sqlite3.connect(str(SESSION_DB), check_same_thread=False)
        _connection.row_factory = sqlite3.Row
        _connection.execute("PRAGMA journal_mode = WAL")
        _connection.execute("PRAGMA busy_timeout = 5000")
        _connection.execute("""
            CREATE TABLE IF NOT EXISTS bot_sessions (
                chat_id INTEGER PRIMARY KEY, order_id TEXT NOT NULL,
...
def _serialize(session) -> dict:
    return {
        "chat_id": session.chat_id, "order_id": session.order_id,
        "user_id": session.user_id, "thread_id": session.thread_id,
        "last_text": session.last_text, "invoice": list(session.invoice or []),
        "task_status": session.task_status, "customer_id": session.customer_id,
        "customer_name": session.customer_name, "kv_invoice_id": session.kv_invoice_id,
        "discount": session.discount, "pvc": session.pvc, "vat": session.vat,
        "kh_debt": session.kh_debt, "payments": list(session.payments or []),
...
def save(session) -> None:
    ...
    try:
        ...
        _conn().execute(
            ...
            (session.chat_id, session.order_id, session.user_id, session.thread_id,
...
def load_all() -> list[dict]:
    try:
        rows = _conn().execute("SELECT data FROM bot_sessions ORDER BY updated_at DESC").fetchall()
