"""bot_core/media.py — Media handling (forward to group)."""
import logging
...
async def handle_callback_media(bot, event):
    ...
    if len(parts) < 4:
        return
    short, original_mid = parts[1], int(parts[2])
    ...
    if s.thread_id:
        try:
            ...
            await bot.send_file(entity, media["file_id"], reply_to=s.thread_id,
    ...
    if short == "soan" and s.thread_id:
        try:
            await post_json(f"{config.ORDER_API_BASE}/api/order/soan",
